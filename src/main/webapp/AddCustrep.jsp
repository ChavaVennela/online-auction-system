...
<%
	...
	rs = st.executeQuery("select * from CustomerReps where username='" + newUserName + "' or email='" + newEmail
	+ "'");
	if (rs.next()) {
	out.println("Sorry the username or user email already exits");
	out.println("<a href='createCustomReps.jsp'>Register</a>");
	...
	/* response.sendRedirect("createNewUser.jsp"); */
		...
		try {
			...
			String insert = "INSERT INTO CustomerReps(name, email, username, password, phone, addr)"
					+ "VALUES (?, ?, ?, ?, ?, ?)";
			...
			out.print("<a href='createCustomReps.jsp'>Register</a>");
		}
	}
%>
